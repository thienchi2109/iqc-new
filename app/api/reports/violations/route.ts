...
import { getViolationsSummary } from '@/lib/query/reports'
...
async function handler(request: NextRequest, user: AuthUser) {
  try {
    ...
    const data = await getViolationsSummary(filters)
    await AuditLogger.logReport(user, 'reports.violations', filters)
    return Response.json({ items: data })
  } catch (error) {
    console.error('reports.violations error', error)
